...
c-----  d(2) : Poisson's ratio nu ---> shear modulus mu
...
c----- d(11) : Reference free volume creation coeff xeta_c
c----- d(12) : Reference temperature theta^*
...
c----- d(18) : Coefficient of thermal conduction
c----- d(19) : Activation energy
c----- d(20) : Boltzmann constant
c----- d(21) : Specific Heat per unit reference volume
c----- d(22) : Threshold stress
c----- d(23) : Stress coefficient
        read(5,*) (d(i),i=1,23)
        write(6,2000) (d(i),i=1,23)
c       e = d(1)
c       pois = d(2)
c---- Store bulk modulus & shear modulus in D(1) & D(2)
...
1000  format(8f10.0)
 2000 FORMAT(/5X,'EKAMBARAM-THAMBURAJA  BMG MODEL',/
     ...
     8 /10X,'COEFFICIENT OF THERMAL CONDUCTION',E10.2
     9 /10X,'ACTIVATION ENERGY: Z',E10.2
     1 /10X,'BOLTZMANN CONSTANT: K_B',E10.2
     2 /10X,'SPECIFIC HEAT CAPACITY : C',E10.2
     3 /10X,'THRESHOLD STRESS : tau_th',E10.2
     4 /10X,'STRESS COEFFICIENT : kbar_th',E10.2)
	end
...
c------ ql(23,5) : d(xi)/ dX_1 at elem centroid
c------ ql(24,5) : d(xi)/ dX_2 at elem centroid
c------ ql(23) : Maximum principal log plastic strain (not used)
c------ ql(24) : Minimum principal log plastic strain (not used)
c
c------ d(1) - d(21) : material parameters as above
c
...
c----- following are material parameters d(1) to d(21)  in E-T model
      ekappa = d(1)
      emu = d(2)
      alfa_th = d(3)
      theta_o = d(4)
      ...
      s3 = d(17)
	tkth= d(18)
	z = d(19)                                                                 !ACTIVATION ENERGY
	tkb = d(20)                                                               !BOLTZMANN CONSTANT
	c = d(21)                                                                 !SPECIFIC HEAT CAPACITY                                                               
	tau_th = d(22)
	tkbar_th = d(23)
...
c      if(time.le.dtime) then 
       if(tau_n.lt.(0.5d0*coh_o)) then                                            !ELASTIC UPDATE
c---- Alternately elastic update can be performed if tau_n < Initial value of cohesion
c---- perform purely elastic update
c
c---- retrieve Fn^p
      call invoigtv(fn1_p,voigt1,ql(11),2)                                      !ql(11)-ql(19) are placed into the tensor fn1_p
c
c---- Invert plastic deformation gradient
	call inverse(Fn1_p,Fn1_p_inv,det_fn1_p)                                   !inverts the tensor fn1_p and puts it into fn1_p_inv
c----  form fn1_e
	do i=1,3
	  do j=1,3
	    fn1_e(i,j)=0.d0
		do k=1,3
		  fn1_e(i,j)=fn1_e(i,j)+fn1(i,k)*fn1_p_inv(k,j)                       !forms the fn1_e matrix
		enddo
	  enddo
	enddo
        call elas_update(d,fn1_e,se,p_n1,tau_n1,tree,theta_n1)                  !tree is trace of Ee, tau_n1 is eq shear stress and p_n1 is the hydrostatic part of Se
c
      xi_dot_on1 = 0.d0 
      gama_dot_n1 = 0.d0
      xi_T = xi_r + ek_theta*(theta_n1 - theta_r)                               !VFT approximation for fully annealed free volume concentration
c 
       else                                                                     !ELASTOPLASTIC UPDATE
c
c---- perform elastic-viscoplastic update
c---- retrieve Fn^p
      call invoigtv(fn_p,voigt1,ql(11),2)                                       !ql(11)-ql(19) are placed into the tensor fn_p
c
c---- Retrieve 2nd P-K stress based on intermediate config
      call invoigtv(se,eps,voigt2,1)                                            !second P-K stress in the form of array eps is placed into tensor se
c
...
c       fnpcoeff2=0.d0
        ...
        temp(3,1) = temp(1,3)
c
c---- Step 3: update F^p i.e. plastic part of deformation gradient                                                  
...
c---- Step 4(a): invert plastic deformation gradient and obtain elastic part of deformation gradient
...
c---- Store F(n+1)^p in  ql
	  call voigtv(fn1_p,voigt1,ql(11),2)                                      !fn1_p is placed into the array ql(11)-ql(19)
c
c     end if
c
c---- Step 8: Compute xi_dot_o(n+1), gama_dot_(n+1)
...
C----- update internal variables
      ql(1) = xi_dot_on1                                                        !xi_dot_n1 and gama_dot_n1 are updated in subroutine gdot
      ql(2) = gama_dot_n1                                                      
      ql(3) = tau_n1                                                            !tau_n1 and p_n1 are updated in subroutine elas_update
      ql(4) = p_n1
      ql(10) = tree - ql(9)                                                     !tree is updated in subroutine elas_update
      ql(9) = tree
      ql(20) = xi_T                                                             !xi_T and coh are updated in subroutine gdot
      ql(22) = coh
c
      ql(25) = tau_v1                                                           !tau_v1 and tau_v2 are updated in subroutine gdot
      ql(26) = tau_v2
c---- update 2nd P-K stress (eps) & Cauchy stress (sig)
      call cauchy (se,sigc,fn1_e)                                               !Cauchy stress tensor sigc is obtained from second P-K stress tensor se
      call voigtv(se,eps,voigt2,1)                                              !Places the second P-K stress tensor into array eps
      call voigtv(sigc,sig,voigt2,1)                                            !Places the Cauchy stress tensor sigc into array sig
...
c---- form lower triangular parts by symmetry
       do i=1,3
       do j=i,3
        ce(j,i) = ce(i,j)
        ee(j,i) = ee(i,j)
...
c---- calculate equivalent stress tau
      tau = soe(1,1)**2.d0 + soe(2,2)**2.d0 + soe(3,3)**2.d0 + 
	12.d0*( soe(1,2)**2.d0 + soe(2,3)**2.d0 + soe(1,3)**2.d0)
      tau = dsqrt(tau/2.d0)
c
      return
      end
...
c
	...
	tkbar_th = d(23)
c
      if(dabs(xi).gt.tol.and.(theta.gt.tol)) then                               !change incorporated
         xi_dot_on1 = f_o*dexp(-z/(tkb*theta)-phi/xi)
      else
         xi_dot_on1 = 0.d0
      end if
...
c        tau_v = tau_v1 
c---- Note we have not ignored -s1*del^2(xi) contribution to viscous stress tau_v 
         f1 = tau - xeta*(tau_v + p)
      
	   coh = coh_o*dexp(ek_c*(xi - xi_o))                                     !change incorporated
c        arg = tau/2.d+08
         if(f1.gt.0.d0) then
         arg = f1/coh
