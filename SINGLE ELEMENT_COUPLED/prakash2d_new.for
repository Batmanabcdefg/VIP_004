...
c---- Coupled mechanics-free volume-temperature evolution analysis: NDF = 4
c
...
c     shap2d & djfn
c
      logical prt,option                                                        !DECLARATION STATEMENTS
      CHARACTER*8 HEAD
      CHARACTER*1 O 
     ...
     1 squad2(4,4,4),pquad2(4,4),squad3(4,4,4),pquad3(4,4)
      DIMENSION QW(9),DJ(7,10),DJM(7,10),p1(8),p2(4),p3(4),s1(8,8),
	1 s2(4,4),s3(4,4)
      DATA PI/3.141592654/
c
      ndfm = 2
...
c---- input material properties
  1   continue                                                                  !STATEMENT 1 FOR INPUTING MATERIAL PARAMETERS
c
...
c---- input model dependent parameters
      ...
      return
c
c----  calculate material gradients of free volume at element centroid
  2   continue                                                                  !STATEMENT 2 FOR COMPUTATION OF GRADIENTS
      ql(23,5) = 0.d0
      ql(24,5) = 0.d0
      CALL SHAP2D(0.D0,0.D0,XL,SHPB,XSJ,NDM,NEL,IX,.TRUE.)                      !shpb-reference configuration
      do i = 1,nel
...
c
c
  3   continue                                                                  !STATEMENT 3 FOR B-BAR FORMULATION
...
c---- compute spatial shape functions
      do 353 i = 1,nel
         ...
         shpb(1,i) = x(1)
         shpb(2,i) = x(2)                                                       !shpb is now in current configuration
353   continue
c---- compute volumetric compatibility matrix
      CALL BMAT(B,XJC,SHPB,XLC,NDM,NDF,NEL,NSTM,MODE)                           !B matrix based on current configuration gradients of shape functions
      do 355 i = 1,nstm
...
c---- Main integration loop over regular quadrature points
      if ((isw.eq.3).or.(isw.eq.7)) then
         do  l = 1,4
            do  j = 1,8
               do  i = 1,8
                squad1(i,j,l) = 0.d0 !DISPLACEMENT DOF
               end do
            end do
            do j=1,4
              do i=1,4
                squad2(i,j,l) = 0.d0 !FREE VOLUME DOF
              end do
            end do
	      do j=1,4
              do i=1,4
                squad3(i,j,l) = 0.d0 !TEMPERATURE DOF
              end do
         end do
	end do
       do i = 1,8
        do j = 1,8
         s1(i,j) = 0.d0 !DISPLACEMENT DOF
        end do
       end do
       do i=1,4
        do j=1,4
         s2(i,j) = 0.d0 !FREE VOLUME DOF
        end do
       end do
	do i=1,4
        do j=1,4
         s3(i,j) = 0.d0 !TEMPERATURE DOF
        end do
       end do
      end if
      if ((isw.eq.6).or.(isw.eq.7)) then
         do  l = 1,4
            do  i = 1,8
              pquad1(i,l) = 0.d0 !DISPLACEMENT DOF
            end do
            do i=1,4
              pquad2(i,l) = 0.d0 !FREE VOLUME DOF
            end do
	      do i=1,4
              pquad3(i,l) = 0.d0 !TEMPERATURE DOF
            end do
         end do
      do i=1,8
       p1(i) = 0.d0 !DISPLACEMENT DOF
       end do
      do i = 1,4
       p2(i) = 0.d0 !FREE VOLUME DOF
      end do
	do i = 1,4
       p3(i) = 0.d0 !TEMPERATURE DOF
      end do
      end if
...
c----  retrieve shape functions
     ...
     2              pquad3(1,l),vel,accel,theta,shpb,bvol,aa,bb,shp,            !shpb-current configuration and shp-reference configuration
...
9001  format(8e13.5)
c----------------------------------------------
      CALL STFSTORE(S,S1,S2,S3,P,P1,P2,P3,NDFM,NDF,NEL,NST,1)
      end if
      if ((isw.eq.6).or.(isw.eq.7)) then
         do  l = 1,4
            do  i = 1,8
	      ...
	      do i=1,4
               p3(i) = p3(i) + pquad3(i,l)*wg(l)
            end do
        end do
        CALL STFSTORE(S,S1,S2,S3,P,P1,P2,P3,NDFM,NDF,NEL,NST,2)
...
C---- OUTPUT ELEMENT VARIABLES
    4 CONTINUE                                                                  !STATEMENT 4 FOR OUTPUTING ELEMENT VARIABLES
...
c      call ivarout(d(6),f,ql(1,ll),X,NDM)
  400 CONTINUE
      RETURN
c
  5   continue                                                                  !STATEMENT 5 FOR FORMATION OF RHS
      mode = d(3)
	c = d(26)                                                                 !change incorporated
      do i=1,8
      p1(i) = 0.d0 !force vector corresponding to displacement DOF
      end do
      do i = 1,4
      p2(i) = 0.d0 !force vector corresponding to free volume DOF
	end do
	do i=1,4
	p3(i)=  0.d0 !force vector corresponding to temperature DOF
      end do
...
C---- COMPUTE SHAPE FUNCTIONS
      CALL SHAP2D(SG(L),TG(L),XL,SHP,XSJ,NDM,NEL,IX,.TRUE.)                     !shp-reference configuration
      IF (MODE.EQ.2) THEN
      RADIUS = 0.D0
      DO 507 I = 1,NEL
...
C---- LUMPED MASS APPROXIMATION
      P1(II) = P1(II) + W1
      P2(I) = P2(I) + W2
	P3(I) = P3(I) + W3                                                        !lumped mass for temperature DOF
...
515   continue
C---- STORE MECH,FREE VOLUME AND TEMPERATURE MASS MXS IN A SINGLE ARRAY P(NST)
      CALL STFSTORE(S,S1,S2,S3,P,P1,P2,P3,NDFM,NDF,NEL,NST,2)
...
c---- update element variables at integration and output points
  9   continue                                                                  !STATEMENT 9 FOR UPDATING INTERNAL VARIABLES
      aa = d(4)
      bb = 1.0d0 - d(4)
      mode = d(3)
...
c*******************************************************************
      subroutine sloop(d,ul,xl,xlc,strl,epsl,ql,ix,tl,s1,p1,p2,p3,vel,
     1                 accel,theta,shpb,bvol,aa,bb,shpu,xsj,                    !shpb-current configuration and shpu-reference configuration
...
c
     ...
     2          dtnum(6,6),bbar(4,18),shp(3,9)
      dimension fpnew(9),snew(9),qnew(100),s1(8,*),p1(*),p2(*),p3(*)            !change incorporated
      DATA PI/3.141592654/
c
...
c---- compute spatial shape functions
      do  i = 1,nel
         ...
         shp(3,i) = shpu(3,i)                                                   !shp-current configuration
      end do
      do i=1,6
       do j=1,6
        dt(i,j) = 0.d0
...
c---- factor = eps + (1-eps)*(theta/xjac)^{1/3}
      ratio = (theta/xjac)**(1.d0/3.d0)
      factor = aa + bb*ratio
c---- reduce deformation gradients
c---- for the case of eps = 0 : F = F*(theta/xjac)^{1/3} => det F = theta = det F(xi = eta = 0)
      f33 = f(3,3)
      do 307 i = 1,3
         do 307 j = 1,3
            f(i,j) = factor*f(i,j)
...
c---- compute compability matrix (B-bar)
      CALL BMAT(B,XJC,SHP,XLC,NDM,NDF,NEL,NSTM,MODE)                            !B matrix based on current configuration gradients
c---- set up dV based on current confign (xsj1) & dV_o reference configuraion (xsj2)
      xsj1 = xsj*xjac
...
c---- factor = (1-eps)*(theta/xjac)^{1/3} /(eps + (1-eps)*(theta/xjac)^{1/3}) 
c            = 1 for case of eps = 0  and = 0 for eps = 1 (no Bbar)
...
c---- scale stresses  & [DT] by dv - remember strl are Cauchy stresses
      do 330 i = 1,6
         sig(i) = xsj1*strl(i)
330   continue
c---------------------------------------------------------------
...
c     K_{iakb} = int_V^e {del_{ik}*sig_{jl}*Na,j*Nb,l} dV^e
...
c----------------------------------------------------------------------
c---- compute current location of radius for (xi,eta) as well as centroid
      IF(MODE.EQ.2)THEN
c---- RAD and rad0 are current radii of Gauss point and centroid
      RAD = 0.d0
      rad0 = 0.d0
      DO 326 I=1,nel
...
c---- K = 2/3{int_V^e {(B^T*sig)x(Bbar^{vol}-B^{vol}) +(Bbar^{vol}-B^{vol})x(B^T*sig)
c                        - sig_{kk}/3*(Bbar^{vol}-B^{vol})x(Bbar^{vol}-B^{vol})}dV^e}
c
...
c
c---- Add foll term : K_{iakb} = int_{V^e} {sig_{kk}/3.0*(Na,k*Nb,i 
c                                            - Na,k*Nb,i|_{reduced GP})} dV^e
...
c
c----- obtain force vector associated with free volume and temperature evolution : p2 and p3
        call pxeta_temp(d(6),ql,p2,p3,shpu,ul,xsj2,ndf,nel)
      end if
      return
      end
c
...
c
c---- compute deformation gradients and jacobian
      call defg(xl,xlc,ul,shp,f,finv,xjac,ndm,ndf,nel,mode,.false.)             !shp-reference configuration
      xsj1 = xsj*xjac
...
c---------------------------------------------------------------
      SUBROUTINE STFSTORE(S,S1,S2,S3,P,P1,P2,P3,NDFM,NDF,NEL,NST,IOP)
C
      IMPLICIT DOUBLE PRECISION (A-H, O-Z)
C---- STORES MECHANICAL,FREE VOLUME AND THERMAL STIFFNESS MATRIX AND FORCE VECTORS
C---- IN GLOBAL MATRIX S AND GLOBAL ARRAY P
      DIMENSION S(NST,*), S1(8,8),S2(4,4),S3(4,4),
     1 P1(8),P2(4),P3(4),P(*)
      IF((IOP.EQ.1).OR.(IOP.EQ.3)) THEN                                         !IOP=1 FOR STIFFNESS MATRIX
      IF (NDF .EQ. 4) THEN
      DO 15 I=1,NEL
      DO 15 K=1,NEL
      DO 10 J=1,2
      DO 10 L=1,2
      S(4*(I-1)+J,4*(K-1)+L) = S1(2*(I-1)+J,2*(K-1)+L)
10    CONTINUE
      S(4*I-1,4*K-1) = S2(I,K)
	S(4*I,4*K) = S3(I,K)                                                      !STIFFNESS TERMS CORRESPONDING TO TEMPERATURE DOF INCORPORATED
15    CONTINUE
...
40    CONTINUE
      END IF
      END IF
      IF((IOP.EQ.2).OR.(IOP.EQ.3)) THEN                                         !IOP=2 FOR FORCE VECTOR
      IF (NDF .EQ. 4) THEN
      DO 30 I=1,NEL
      DO 25 J=1,2
      P(4*(I-1)+J) = P1(2*(I-1)+J)
25    CONTINUE
      P(4*I-1) = P2(I)
	P(4*I) = P3(I)                                                            !FORCE VECTOR TERMS CORRESPONDING TO TEMPERATURE DOF
30    CONTINUE
      ELSEIF (NDF .EQ. 2) THEN
      DO 35 I=1,NST
      P(I) = P1(I)
...
45    CONTINUE
      ...
      END
c-----------------------------------------------------------------------
      subroutine pxeta_temp(d,ql,p2,p3,shpu,ul,xsj2,ndf,nel)
      implicit double precision (a-h,o-z)
	common /tdata/ time,dtime,bet1,gam1
      dimension d(*),ql(*),p2(*),p3(*),shpu(3,*),ul(ndf,*),sxi(4,4),
	1stheta(4,4),delxi(2),delxid(2),p31(4)
c----- following are material parameters d(1) to d(18)  in E-T model
      ekappa = d(1)
      emu = d(2)
      alfa_th = d(3)
      theta_o = d(4)
      ...
      s2 = d(16)
      s3 = d(17)
	tkth= d(18)                        !coefficient of thermal conduction
	tau_th = d(22)
	tkbar_th = d(23)
...
c---- following are the internal variables
	...
	if(tau.lt.tau_th) then
	xeta = xeta_c
	else
	xeta = xeta_c + tkbar_th*(tau-tau_th)
	endif     
...
c-----form [K_theta] matrix
	...
	const4 = (3.d0*ekappa*alfa_th*del_trE_e + 
     2 s2*ek_theta*del_xi_n)/dtime                                              !change incorporated

	do i=1,nel
	do j=1,nel
	stheta(i,j)=stheta(i,j) + tkth*(shpu(1,i)*shpu(1,j)+
	1 shpu(2,i)*shpu(2,j)) + const4*shpu(3,i)*shpu(3,j)
	enddo
	enddo

	do i=1,nel
      do j=i,nel
      stheta(j,i) = stheta(i,j)
      end do
      end do

c-----form [K_theta]*{theta}
      do i=1,nel
	do j=1,nel
	p3(i) = p3(i) - stheta(i,j)*ul(4,j)
	enddo
	enddo

c-----form {F_theta^int}
      const5 = tau*gama_dot-del_xi_n*(s2*(xi_n1-xi_T) + p)/dtime
      const6 = 0.d0                                                             !change incorporated

	do j=1,2
	delxi(j) = 0.d0
	delxid(j) = 0.d0
	do i = 1,nel
	delxi(j) = delxi(j) + shpu(j,i)*ul(3,i)
	delxid(j) = delxid(j) + shpu(j,i)*ul(3,i+nel)
	enddo
	const6 = const6 - delxi(j)*delxid(j)
	enddo                                                                     !change incorporated
	
	const5 = const5- s1*const6
	const7 = s1*del_xi_n/dtime 
      
	do i=1,nel
	p31(i) = shpu(1,i)*delxi(1) + shpu(2,i)*delxi(2)
	enddo
	 
	do i=1,nel                                                                !change incorporated
        p3(i) = p3(i) + shpu(3,i)*const5 - p31(i)*const7
      end do
