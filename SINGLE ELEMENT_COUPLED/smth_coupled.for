...
c---- compute NT*N
  ...
  326 continue
  300 continue
      CALL STFSTORE(S,S1,S2,s2,P,P1,P2,p2,NDFM,NDF,NEL,NST,1)
      return
c
...
c     p(2*i-1) = p(2*i-1) + epsl(id3,l)*w
      ...
      if(id2.ne.0) then
      if(id2.le.100) then
      p1(2*i) = p1(2*i) + strl(id2,l)*w
      end if
      if(id2.gt.100) then
      if(id2.le.200) then
      id3 = id2 - 100 
      p1(2*i) = p1(2*i) + ql(id3,l)*w
      else
       id3 = id2 - 200
      p1(2*i) = p1(2*i) + ql(id3,l)*w
c     id3 = id2 - 13
...
c     p(2*i) = p(2*i) + ang*w
  ...
  605 continue
  600 continue
      CALL STFSTORE(S,S1,S2,s2,P,P1,P2,p2,NDFM,NDF,NEL,NST,2)
      return
c
c---- unused isw
    8 continue
