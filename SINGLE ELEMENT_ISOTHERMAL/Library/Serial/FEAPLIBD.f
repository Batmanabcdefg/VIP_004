C
C     SUBROUTINE ELMT01(D,UL,XL,STRL,EPSL,QL,IX,TL,S,P,VEL,ACCEL,
C    *                  NDF,NDM,NST,NS,NQ,ISW)
C     IMPLICIT DOUBLE PRECISION (A-H,O-Z)
C
C---- DUMMY ELEMENT ROUTINE
C
C     RETURN
C     END
      SUBROUTINE ELMT02(D,UL,XL,STRL,EPSL,QL,IX,TL,S,P,VEL,ACCEL,
     *                  NDF,NDM,NST,NS,NQ,ISW)
...
C
      RETURN
      END
      SUBROUTINE ELMT03(D,UL,XL,STRL,EPSL,QL,IX,TL,S,P,VEL,ACCEL,
     *                  NDF,NDM,NST,NS,NQ,ISW)
      IMPLICIT DOUBLE PRECISION (A-H,O-Z)
C
C---- DUMMY ELEMENT ROUTINE
...
C
C---- FORM 8-NODE BRICK SHAPE FUNCTIONS
      DO 10 I = 1,8
      SHPR = 0.5 + R(I)*RR
      SHPS = 0.5 + S(I)*SS
      SHPT = 0.5 + T(I)*TT
...
C
C---- CONSTRUCT JACOBIAN AND INVERSE
      DO 20 I = 1,3
      DO 20 J = 1,3
      XS(I,J) = 0.D00
      DO 20 K = 1,8
...
C---- 1ST ORDER QUADRATURE
   10 RR(1) = 0.D0
      ...
      RETURN
C
C---- 2ND ORDER QUADRATURE
   20 CONST = 1./DSQRT(3.D0)
C  20 CONST = 1./SQRT(3.E0)
      DO 30 I = 1,8
      RR(I) = R(I)*CONST
      SS(I) = S(I)*CONST
      TT(I) = T(I)*CONST
...
C     G4(1) = SQRT((3.0E0+G)/7.)
      G4(4) = - G4(1)
      G4(2) = DSQRT((3.0D0-G)/7.)
C     G4(2) = SQRT((3.0E0-G)/7.)
      G4(3) = -G4(2)
      H4(1) = 0.5 - H
      H4(2) = 0.5 + H
      H4(3) = 0.5 + H
...
10    LINT = 3
      G = DSQRT(1.0D0/3.0D0)
C     G = SQRT(1.0E0/3.0E0)
      H = DSQRT(2.0D0/3.0D0)
C     H = SQRT(2.0E0/3.0E0)
      R(1) = -H
      R(2) =  H
      R(3) = 0.
      Z(1) = -G
...
C
      ...
      IMPLICIT DOUBLE PRECISION (A-H,O-Z)
C
C---- GAUSS POINTS AND WEIGHTS FOR 1 DIMENSION (BEAM ELEMENT)
C
      DIMENSION R(*),W(*)
      GO TO (1,2,3) LINT
C---- 1 POINT QUAD
...
C   9 ROWMAX = AMAX1(ROWMAX,ABS(W(I,J)))
C     IF (ROWMAX.EQ.0.0) GO TO 999
      IF (DABS(ROWMAX).LT.TOL) GO TO 999
C     IF (ABS(ROWMAX).LT.TOL) GO TO 999
   10 D(I) = ROWMAX
...
C     COLMAX = ABS(W(IP,K))/D(IP)
      DO 11 I = KP1,N
      IP = IPIVOT(I)
      AWIKOV = DABS(W(IP,K))/D(IP)
C     AWIKOV = ABS(W(IP,K))/D(IP)
      IF (AWIKOV.LE.COLMAX) GO TO 11
      COLMAX = AWIKOV
      J = I
   11 CONTINUE
C     IF (COLMAX.EQ.0.0) GO TO 999
      IF (DABS(COLMAX).LT.TOL) GO TO 999
C     IF (ABS(COLMAX).LT.TOL) GO TO 999
      IPK = IPIVOT(J)
      IPIVOT(J) = IPIVOT(K)
      IPIVOT(K) = IPK
      DO 20 I = KP1,N
...
C     IF (W(IP,N).EQ.0.0) GO TO 999
      IF (DABS(W(IP,N)).LT.TOL) GO TO 999
C     IF (ABS(W(IP,N)).LT.TOL) GO TO 999
      RETURN
  999 FLAG = .FALSE.
      RETURN
      END
...
C
C---- SOLVE A*X = B AND RETURN X IN B
C
...
c      SUBROUTINE MA01(UL,XL,TL,LD,P,S,IE,D,ID,X,IX,F,T,JDIAG,
c     1 STR,EPS,Q,B,DR,VELG,ACCELG,VEL,ACCEL,CT,NDF,NDM,NEN1,
c     2 NST,NSTR,NQ,NEND,FLG)
c      IMPLICIT DOUBLE PRECISION (A-H,O-Z)
c 
c      LOGICAL FLG
c 
...
C---- WRITE INPUT/OUTPUT FILES FOR G-SAFE PRE-POST PROCESSOR
      IF(KEY.EQ.0) THEN
C     OPEN(UNIT=51,FILE='CDATA.IN1',FORM='FORMATTED',ACCESS=
C    1 'SEQUENTIAL')
C     OPEN(UNIT=52,FILE='CBOUND.IN1',FORM='FORMATTED',ACCESS=
C    1 'SEQUENTIAL')
      OPEN(UNIT=53,FILE='CGEOM.IN1',FORM='FORMATTED',ACCESS=
     1 'SEQUENTIAL')
C     OPEN(UNIT=54,FILE='CMATS.IN1',FORM='FORMATTED',ACCESS=
C    1 'SEQUENTIAL')
C     OPEN(UNIT=55,FILE='ELODIN.IN3',FORM='FORMATTED',ACCESS=
C    1 'SEQUENTIAL')
C     OPEN(UNIT=56,FILE='POST.INF',FORM='FORMATTED',ACCESS=
C    1 'SEQUENTIAL')
C     OPEN(UNIT=57,FILE='STRIN.IN9',FORM='FORMATTED',ACCESS=
C    1 'SEQUENTIAL')
      IDUM = 0
...
601   CONTINUE
      ...
      IJ = (IQUAD-1)*NQDM
C     JJ = IQUAD*NQDM
C     WRITE(63,6003)IQUAD,Q(JJ-1,N),Q(JJ,N),STR(II+2,N),STR(II+3,N),
C---- PLOT STRESS CONTOURS
      SIG(5)=0.D0
      SIG(6)=0.D0
C---- EFFECTIVE STRESS
      DO 463 I=1,NSDM
...
C---- WRITE NODAL COORDS AND NODAL DISPLACEMENTS
      WRITE(LPLOT) (X(1,J),J=1,NUMNP),(X(2,J),J=1,NUMNP),
     1 (B(I),I=1,NEQ)
C---- WRITE NODAL CONNECTIVITY ARRAYS FOR ALL ELEMENTS
...
10    CONTINUE
      NQD1 = (NQUAD-1)*NSDM
C---- WRITE NODAL STRESSES, MISES EQV STRESS AND HYDROSTATIC STRESS
C---- NODAL STRESSES ARE OBTAINED BY AVERAGING OVER ALL ELEMENTS MEETING
C---- AT A NODE
      NSD2 = NSDM + 2
      DO 20 I=1,NSD2
      DO 30 N=1,NUMEL
...
C     SIG = SQRT(3.0*SIG/2.0)
      ...
      DO 35 J=1,NEN
      II = IX(J,N)
C---- II IS GLOBAL NODE # CORR TO LOCAL NODE J
C---- ADD CONTRIBUTION TO GLOBAL NODE II
      DR(II) = DR(II) + SIG
C---- INCREMENT COUNTER FOR # CONTRIBUTIONS FROM ELEMS SURROUNDING
C---- GLOBAL NODE II
...
45    CONTINUE
20    CONTINUE
C---- WRITE NODAL STRAINS. NODAL STRAINS ARE OBTAINED BY ELEM AVERAGING
      DO 120 I=1,NSDM
      DO 130 N=1,NUMEL
      SIG=EPS(NQD1+I,N)
      DO 135 J=1,NEN
...
145   CONTINUE
120   CONTINUE
C---- WRITE INTERNAL VARIABLES 1 TO 3
      NQD2 = (NQUAD-1)*NQDM
      DO 220 I=1,7
      DO 230 N=1,NUMEL
      SIG = Q(NQD2+I,N)
...
220   CONTINUE
      ...
      END
C
C     SUBROUTINE MP01(IDL,IE,D,ID,X,IX,F,T,DR,STR,EPS,Q,VEL,ACCEL,
C    1 NDF,NDM,NEN1,NSTR,NQ,III,PRTN)
C     IMPLICIT DOUBLE PRECISION (A-H,O-Z)
C
C     LOGICAL PRTN
C
...
C
     ...
     2 NST,NSTR,NQ,NEND,FLG)
C---- DYNAMIC ENERGY RELEASE RATE (DOMAIN INTEGRAL STATIONARY CRACK)
      IMPLICIT DOUBLE PRECISION (A-H,O-Z)
...
C---- OUTPUT
C     INQUIRE(JDUMP,OPENED=DUMPJ)
C     IF(.NOT.DUMPJ) THEN
C      OPEN(JDUMP,FILE='FPJINT',STATUS='NEW',FORM='FORMATTED')
C     END IF
C
      DO 418 NDI = 1,NDOM(3)
      DO 418 K = 1,NDOM(1)
C---- DOUBLE FOR SYMMETRY
      IF(NDOM(2).NE.0) THEN
       DO 414 I = 1,7
C      DYNJM(NDI,I,K) = DYNJM(NDI,I,K)*2.0
 414   DYNJ(NDI,I,K) = DYNJ(NDI,I,K)*2.D0
      ENDIF
C---- DIVIDE BY VCE AREA (FOR THREE-DIMENSIONAL CRACK)
      DO 416 I = 1,7
C     DYNJM(NDI,I,K) = DYNJM(NDI,I,K)/XC(NDI,4)
...
C     IF(K.NE.1) DYNJM(NDI,4,1) = DYNJM(NDI,4,1) + DYNJM(NDI,4,K)
      IF(CT(1).NE.XKEY1)
     1  WRITE(JDUMP,'(2I3,7(1PG12.4))') NDI,K,(DYNJ(NDI,J,K),J=1,7)
C     IF(CT(1).NE.XKEY1)
C    1  WRITE(JDUMP,'(2I3,5(1PG12.4))') NDI,K,(DYNJM(NDI,J,K),J=1,4)
 418  CONTINUE
C     WRITE(JDUMP,'(7(1PE12.4)/7(1PE11.3))') TIME,
C    1  (DYNJ(NDI,6,1)/FLOAT(NDOM(1)),NDI=1,NDOM(3)),
C    2  (DYNJ(NDI,7,1)/FLOAT(NDOM(1)),NDI=1,NDOM(3)),TSWE,TKEE,TKEE1
...
C     WRITE(JDUMP,'(F9.3,6(1PE11.3)/7(1PE11.3))') TIME,
C    1  (DYNJM(NDI,4,1)/FLOAT(NDOM(1)),NDI=1,NDOM(3)),TSWE,TKEE
      RETURN
      END
C
      SUBROUTINE QWT(XL,QW,NDM,NEL,N,LDOM)
C
C---- 2 - D ENERGY RELEASE RATE FOR MODE I STATIONARY CRACK
C
C---- COMPUTE WEIGHTS QW
C
      IMPLICIT DOUBLE PRECISION(A-H,O-Z)
     ...
     1 DYNJM(8,7,10),TSWE,TKEE,TKEE1
      ...
      DIMENSION XL(NDM,*),QW(9)
C
      XIFN = DOM(4,N)
      IF(XIFN.GT.1.D0) GOTO 2
C---- BILINEAR FUNCTION (RECTANGULAR DOMAIN)
      XX = DOM(1,N)
      YY = DOM(2,N)
...
C     B = ABS(B)
 ...
 10   CONTINUE
      RETURN
C---- CONICAL FUNCTION (CIRCULAR DOMAIN)
    2 CONTINUE
      RR = DOM(1,N)
      IF (DOM(3,N).EQ.0.D0) THEN
       RI = 0.D0
...
C     A  = SQRT((X - XC(1,1))**2 + (Y - XC(1,2))**2)/RR
      IF (A.LT.1.D0) THEN
...
C
     ...
     1 IDOM,DJ,DJM)
C---- 2 - D AREA INTEGRAND FOR J  STATIONARY CRACK
      IMPLICIT DOUBLE PRECISION(A-H,O-Z)

      COMMON /JAUX/ IAUX,STRAUX(4),EPSAUX(2),XCT(2),RAD,MODE
      DIMENSION UL(NDF,*),SIG(*),SHP(3,9),ACCEL(*),DJ(7,*),DJM(7,*),
     1 QW(*),VEL(*)
...
C    1           + SIG(4)*(EPSX(1)*DELQ(2) + EPSX(2)*DELQ(1))
     1           + SIG(4)*EPSX(1)*DELQ(2) + SIG(5)*EPSX(2)*DELQ(1)
C---- MODIFICATION-BEGIN ( HOOP STRESS CORRECTION REF: DELORENZI
C---- pp. 129-143, Vol.21, 1985 EFM)
           IF(MODE.EQ.2) THEN
...
C    1           + SIG(4)*(EPSY(1)*DELQ(2) + EPSY(2)*DELQ(1))
...
C
C---- 3 - D ENERGY RELEASE RATE FOR MODE I STATIONARY CRACK
C
C---- COMPUTE WEIGHTS QW
C
      IMPLICIT DOUBLE PRECISION (A-H,O-Z)
...
C     A  = SQRT((X - XC(NDI,1))**2 + (Y - XC(NDI,2))**2)/RR
      IF(A.LT.1.D0) THEN
       IF(A.GT.RI) QW(I) = (1.D0 - A)/(1.D0 - RI)
       LDOM = 1
      ELSE
...
C---- AREA INTEGRAND FOR DYNAMIC J, STATIONARY CRACK
      IMPLICIT DOUBLE PRECISION (A-H,O-Z)
...
c    2           + SIG(4)*(EPSX(1)*DELQ(2) + EPSX(2)*DELQ(1))
c    3           + SIG(5)*(EPSX(2)*DELQ(3) + EPSX(3)*DELQ(2))
c    4           + SIG(6)*(EPSX(3)*DELQ(1) + EPSX(1)*DELQ(3))
...
C---- TOTAL CONTRIBUTION
c     DJ(4,IDOM) = DJ(1,IDOM) - DJ(2,IDOM) + DJ(3,IDOM)
      DJ(6,IDOM) = DJ(1,IDOM) - DJ(2,IDOM) + DJ(3,IDOM)
      RETURN
      END
      ...
      implicit double precision (a-h,o-z)
C
C     J-INTEGRAL CALCULATION BY LINE INTEGRATION FOR CHARPY V NOTCH SPECIMEN.
C     READS THE VALUE OF EACH VARIABLE AT O/P QUAD PTS & USES THE AVERAGE VALUE
C     BETWEEN TWO QUAD PTS TO CALC THE VALUE OF THE INTEGRAL OVER THAT SEGMENT
C     AND SUM IT UP
C     INCORPORATED BY MANISH JHA 'OCTOBER 1993'
...
C     CALCULATE SEGMENT LENGTH
      ...
      D=DSQRT(SD)
C     CALCULATE SEGMNT NORMAL DIRN COSINES
      XN1=(X2(I+1)-X2(I))/D
      XN2=-(X1(I+1)-X1(I))/D
      DEN=DSQRT((XN1**2)+(XN2**2))
      XN1=XN1/DEN
      XN2=XN2/DEN
C     CALCULATE AVERAGES OVER THE SEGMENT
...
C     FIRST TERM BETWEEN BOUNDARY AND 1ST QUAD PT.,ASSUMING QUAD PT.
C     VALUES OVER THIS SEGMENT
      IF(I.EQ.1) THEN
C     D=DIST BETWWEN THE FIRST QUAD PT AND THE INCLINED NOTCH FACE
      D=(1/(2.613))*((2.414*X1(1))+X2(1)-5.0790)
      D=DABS(D)
      FT=(WSE1(1)*XN2-(S11(1)*XN1*DU12(1)+S12(1)*XN2*DU12(1)+
     #     S12(1)*XN1*DU22(1)+S22(1)*XN2*DU22(1)))*D
      XJ=XJ+FT
C     LAST TERM BETWWEN BOUNDARY AND LAST QUAD PT.
      ELSE IF(I.EQ.NS) THEN
      TL=(WSE1(NP)-(S12(NP)*DU12(NP)+S22(NP)*DU22(NP)))*X1(NP)
      XJ=XJ+TL
      ENDIF
100   CONTINUE
C     BECAUSE OF SYMMETRICITY OF THE PROBLEM
      XJ=XJ*2
      WRITE(99,12) TIME, XJ
...
C
      ...
      implicit double precision (a-h,o-z)
c
c  6-node composite triangular element (piecewise linear)
c
      logical flag
      ...
      data tvol/0.d0/
c
c     write(6,*)'*** composite shape fn derivatives'
c
...
c
     ...
     1    - c33*c12**2
c
c     write(6,1001)c11,c22,c33,c12,c13,c23,det
c1001 format('cij=',1p6g12.4,/,'det=',1pg12.4)      
c
      d1 = c22*c33 - c23*c23
      d2 = c13*c23 - c12*c33
      d3 = c12*c23 - c13*c22
...
c        write(6,1002)isub,a(isub),b(isub),c(isub)
c1002    format('isub,a,b,c=',i4,1p3g12.4)
      end do
c
...
c
c     write(6,1003)((shp(i,j),j=1,6),i=1,2)
c1003 format('shp()=',1p6g12.4)
c
      xsj = c11/3.d0 
c
      tvol = tvol + xsj
c     write(6,*)'xsj,tvol=',xsj,tvol
c
      return
      end
c
c
      ...
      implicit double precision (a-h,o-z)
c
c---- shape fns and derivatives for 6-node triangle (quadratic)
c
      logical flag
...
c
      ...
      implicit double precision (a-h,o-z)
c
c---- shape fns and derivatives for 10-node tetrahedran (quadratic)
c
      logical flag
